...
// ====== Concurrency limit for 400-500 users ======
const MAX_CONCURRENT_STREAM_FETCHES = 50; // limit parallel upstream fetches
...
// ====== SECURITY: Rate Limiter (tuned for 400-500 users) ======
const rateLimitMap = new Map<
  string,
  { count: number; resetTime: number }
>();
const RATE_LIMIT_WINDOW = 60_000;
const RATE_LIMIT_MAX = 200; // increased for HLS segment requests
const BLOCK_THRESHOLD = 800; // increased threshold
...
// ====== SECURITY: Response Headers (FIXED: CSP updated for streaming) ======
function securityHeaders(): Record<string, string> {
  return {
    ...
    "Content-Security-Policy":
      ...
      "connect-src 'self' blob:; " + // FIXED: added blob: for HLS.js worker
      "worker-src 'self' blob:; " + // FIXED: needed for HLS.js web worker
...
// ====== CORS Headers for stream resources ======
...
// ====== Stream Proxy: HLS streams (COMPLETELY REWRITTEN) ======

// FIXED: Use streaming pass-through for TS segments instead of buffering in memory
// Only cache playlist (small text), NOT TS segments (large binary)

const hlsPlaylistCache = new Map<
  string,
  { data: Uint8Array; contentType: string; expires: number }
>();
const HLS_PLAYLIST_TTL = 1_500; // 1.5s for live playlist freshness
const HLS_PLAYLIST_CACHE_MAX = 500;

setInterval(() => {
  const now = Date.now();
  for (const [key, entry] of hlsPlaylistCache) {
    if (now > entry.expires) hlsPlaylistCache.delete(key);
  }
}, 5_000);
...
const streamTokens = new Map<
  string,
  { baseUrl: string; m3u8Path: string; created: number; originalUrl: string; allowedDomains: Set<string> }
>();
const STREAM_TOKEN_TTL = 90 * 60 * 1000;
...
function registerStreamUrl(streamUrl: string): string {
  ...
  // Extract allowed domains (main domain + common CDN patterns)
  const mainDomain = urlObj.hostname.split(".").slice(-2).join(".");
  const allowedDomains = new Set<string>([mainDomain]);
  ...
  console.log(`[STREAM] Token: ${token.substring(0, 8)}... for ${streamUrl.substring(0, 60)}...`);
  return token;
}

// FIXED: Resolve URL properly handling all relative/absolute cases
function resolveUrl(base: string, relative: string): string {
  ...
  try {
    ...
    baseParts.pop(); // remove last segment (filename)
    const resolvedPath = baseParts.join("/") + "/" + relative;
...
// FIXED: Fetch playlist only (small text, safe to cache in memory)
async function fetchPlaylist(
  fullUrl: string,
  retryCount: number = 0
): Promise<{ data: Uint8Array; contentType: string } | null> {
  const cached = hlsPlaylistCache.get(fullUrl);
  if (cached && Date.now() < cached.expires) {
    return { data: cached.data, contentType: cached.contentType };
  }
  ...
  try {
    ...
    if (!res.ok) {
      if ((res.status === 404 || res.status === 403 || res.status === 502) && retryCount < 2) {
        await new Promise(r => setTimeout(r, 500 + retryCount * 500));
        return fetchPlaylist(fullUrl, retryCount + 1);
      }
      console.warn(`[STREAM] Playlist fetch failed: ${res.status} ${fullUrl.substring(0, 60)}`);
      return null;
    }
    ...
    // Enforce max size for playlists (should be small text)
    if (data.length > 1 * 1024 * 1024) {
      console.warn(`[STREAM] Playlist too large: ${data.length} bytes`);
      return { data, contentType };
    }

    if (hlsPlaylistCache.size >= HLS_PLAYLIST_CACHE_MAX) {
      let oldestKey: string | null = null;
      let oldestTime = Infinity;
      for (const [key, entry] of hlsPlaylistCache) {
        if (entry.expires < oldestTime) {
      ...
      if (oldestKey) hlsPlaylistCache.delete(oldestKey);
    }

    hlsPlaylistCache.set(fullUrl, { data, contentType, expires: Date.now() + HLS_PLAYLIST_TTL });
    return { data, contentType };
  } catch (e) {
    if (retryCount < 2) {
      await new Promise(r => setTimeout(r, 500 + retryCount * 500));
    ...
    console.error(`[STREAM] Playlist fetch error after retries: ${fullUrl.substring(0, 60)}`, e);
    return null;
  }
}

// FIXED: Stream TS segments directly (pass-through proxy, no memory buffering)
async function proxySegmentStream(
  fullUrl: string,
  retryCount: number = 0
): Promise<Response | null> {
  // Concurrency gate
  if (activeStreamFetches >= MAX_CONCURRENT_STREAM_FETCHES) {
    return new Response("Server busy, try again", { status: 503, headers: { "Retry-After": "2" } });
  }
  ...
  try {
    ...
    if (!res.ok) {
      activeStreamFetches--;
      if ((res.status === 404 || res.status === 502) && retryCount < 1) {
        await new Promise(r => setTimeout(r, 800));
        return proxySegmentStream(fullUrl, retryCount + 1);
      }
      return null;
    }
    ...
    // Create a TransformStream that decrements counter when done
    const { readable, writable } = new TransformStream();
    const pipePromise = res.body.pipeTo(writable).finally(() => {
      activeStreamFetches--;
    });
    ...
    const headers: Record<string, string> = {
      ...
      "Cache-Control": "public, max-age=300", // browser can cache segments
      ...corsHeaders(),
    };
    if (contentLength) {
    ...
    console.error(`[STREAM] Segment error: ${fullUrl.substring(0, 60)}`, e);
    return null;
  }
}

// FIXED: Completely rewritten M3U8 rewriter with proper URL resolution
function rewriteM3u8Playlist(
  ...
  for (const rawLine of lines) {
    const line = rawLine.trimEnd(); // preserve leading whitespace if any, trim trailing
    ...
    if (line.startsWith("#")) {
      ...
      if (line.includes("URI=")) {
        const rewrittenLine = line.replace(/URI="([^"]+)"/g, (_match, uri) => {
          const fullUri = resolveUrl(contextBaseUrl + "/placeholder", uri);
          return `URI="/api/stream/${streamToken}/resource?url=${encodeURIComponent(fullUri)}"`;
        });
    ...
    // Non-comment, non-empty = URL line (segment or sub-playlist)
    const trimmed = line.trim();
    const fullUrl = resolveUrl(contextBaseUrl + "/placeholder", trimmed);
    rewritten.push(`/api/stream/${streamToken}/resource?url=${encodeURIComponent(fullUrl)}`);
  }

  return new TextEncoder().encode(rewritten.join("\n"));
}

// FIXED: Domain validation that allows CDN subdomains
function isDomainAllowed(streamInfo: { baseUrl: string; allowedDomains: Set<string> }, resourceUrl: string): boolean {
  try {
    const resourceHost = new URL(resourceUrl).hostname;
    const resourceDomain = resourceHost.split(".").slice(-2).join(".");

    // Check against allowed domains
    if (streamInfo.allowedDomains.has(resourceDomain)) return true;

    // Check against base URL domain
    const baseDomain = new URL(streamInfo.baseUrl).hostname.split(".").slice(-2).join(".");
    if (resourceDomain === baseDomain) return true;

    // Auto-allow and remember CDN-like domains (common patterns)
    const cdnPatterns = [/cdn/i, /stream/i, /media/i, /video/i, /hls/i, /live/i, /edge/i, /akamai/i, /cloudfront/i, /fastly/i];
    if (cdnPatterns.some(p => p.test(resourceHost))) {
      streamInfo.allowedDomains.add(resourceDomain);
      console.log(`[STREAM] Auto-allowed CDN domain: ${resourceDomain}`);
      return true;
    }
...
async function backgroundRefreshMatches() {
  try {
    ...
    for (const match of allMatches) {
      if (match.servers) {
        for (const server of match.servers) {
          if (server.stream_url && server.stream_url.startsWith("http")) {
    ...
    console.log(
      `[BG] Refreshed: ${allMatches.length} matches (live: ${allMatches.filter((m: any) => m.match_status === "live").length}, tokens: ${streamTokens.size}, fetches: ${activeStreamFetches})`
    );
  } catch (e) {
...
serve(async (req) => {
  ...
  if (url.pathname.match(/^\/api\/stream\/([a-f0-9]+)\/playlist\.m3u8$/)) {
    ...
    const result = await fetchPlaylist(fullUrl);
    if (!result) {
  ...
  // --- API ROUTE: Stream Proxy (sub-resources: playlists + segments) ---
  if (url.pathname.match(/^\/api\/stream\/([a-f0-9]+)\/resource$/)) {
    ...
    try {
      ...
      if (!fullResourceUrl.startsWith("http")) {
        const origin = new URL(streamInfo.baseUrl).origin;
        fullResourceUrl = fullResourceUrl.startsWith("/")
          ? origin + fullResourceUrl
          : streamInfo.baseUrl + "/" + fullResourceUrl;
      }

      // Domain validation
      if (!isDomainAllowed(streamInfo, fullResourceUrl)) {
        console.warn(`[STREAM] Blocked domain: ${new URL(fullResourceUrl).hostname}`);
    ...
    // Determine if this is a playlist or a segment
    const urlPath = fullResourceUrl.split("?")[0];
    const isPlaylist = urlPath.endsWith(".m3u8") || urlPath.endsWith(".m3u");

    if (isPlaylist) {
      // Fetch and rewrite sub-playlist
      const result = await fetchPlaylist(fullResourceUrl);
      if (!result) {
        return new Response("Playlist unavailable", {
          status: 502,
          headers: { ...securityHeaders(), ...corsHeaders() },
        });
      }

      // Derive base URL from the sub-playlist URL
      const urlObj = new URL(fullResourceUrl);
      const pathParts = urlObj.pathname.split("/");
      pathParts.pop();
      const subBaseUrl = urlObj.origin + pathParts.join("/");

      const rewritten = rewriteM3u8Playlist(result.data, token, subBaseUrl);
      return new Response(rewritten, {
        status: 200,
        headers: {
          "Content-Type": "application/vnd.apple.mpegurl",
    ...
    // FIXED: For segments (.ts, .aac, .mp4, .cmfv, .cmfa, etc.) use streaming pass-through
    const streamResponse = await proxySegmentStream(fullResourceUrl);
    if (!streamResponse) {
  ...
  if (url.pathname === "/api/stats") {
    ...
    for (const [, entry] of hlsPlaylistCache) {
    ...
    return new Response(
      JSON.stringify({
        ...
        playlist_cache_entries: hlsPlaylistCache.size,
        stream_tokens_active: streamTokens.size,
        active_stream_fetches: activeStreamFetches,
        max_concurrent_fetches: MAX_CONCURRENT_STREAM_FETCHES,
  ...
  // FIXED: Deno serve options for handling many concurrent connections
  ...
  <script>
    ...
    async function load() {
      ...
      try {
        ...
        scheduleNextRefresh(); // adjust interval on success
        ...
        scheduleNextRefresh(); // adjust interval on failure
    ...
    // FIXED: Completely rewritten HLS player with proper error recovery
    function play(streamUrl) {
      ...
      // Fully clean up previous player
      if (currentHls) {
        currentHls.destroy();
        currentHls = null;
      }
    ...
    function startHlsPlayback(streamUrl, vid) {
      if (typeof Hls !== "undefined" && Hls.isSupported()) {
        var hls = new Hls({
          ...
          maxBufferSize: 20 * 1000 * 1000, // 20MB - conservative for many users
          maxBufferHole: 0.5,
          ...
          xhrSetup: function(xhr, url) {
            // Ensure proper request headers for proxied streams
            xhr.timeout = 15000;
          }
        });
        currentHls = hls;
        ...
        // Loading timeout - if manifest doesn't load in 20s, show error
        ...
        hls.on(Hls.Events.ERROR, function(event, data) {
          ...
          var now = Date.now();
          // Reset error count if more than 30s since last error
          ...
          if (data.type === Hls.ErrorTypes.NETWORK_ERROR) {
            ...
            // Fallback: full reload if still stuck after 10s
            setTimeout(function() {
              if (currentHls === hls && vid.paused && vid.readyState < 2) {
                console.log("[HLS] Still stuck, doing full reload...");
                hls.destroy();
                currentHls = null;
            ...
            if (fatalErrorCount <= 2) {
              ...
              // Swap audio codec on third attempt
              hls.swapAudioCodec();
              hls.recoverMediaError();
            }
          } else {
    ...
    document.addEventListener("visibilitychange", function() {
      if (document.hidden) {
        ...
        // Immediate refresh + resume normal schedule when tab visible
        load();
        scheduleNextRefresh();
      }
    });
