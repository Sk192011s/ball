...
// ====== SECURITY: Rate Limiter (tuned for heavy logo loading on 400-500 users) ======
const rateLimitMap = new Map<
  string,
  { count: number; resetTime: number }
>();
const RATE_LIMIT_WINDOW = 60_000;
const RATE_LIMIT_MAX = 500; // Increased to allow high logo fetching
const BLOCK_THRESHOLD = 1000; // Increased threshold
...
// ====== SECURITY: Response Headers (CSP updated to allow direct streams) ======
function securityHeaders(): Record<string, string> {
  return {
    ...
    "Content-Security-Policy":
      ...
      "media-src * blob: https: http:; " + // ALLOW direct video streaming from any domain
      "connect-src * blob: data:; " + // ALLOW hls.js to fetch from external domains directly
...
<body>
  ...
  <script>
    ...
    // Direct HLS streaming logic without proxy
    function play(streamUrl) {
      if (!streamUrl || typeof streamUrl !== "string") return;
      if (!/^(https?:\/\/)/i.test(streamUrl)) return; // Ensure it's a valid external URL
